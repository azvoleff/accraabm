...
DATA_PATH <- "R:/Data/AccraABM/Runs/Default/20120210-232115_azvoleff-THINK"
...
plot_health<- function(DATA_PATH, timestep) {
    ...
    kriglocations <- raster(paste(DATA_PATH, "AccraABM_world_mask.tif", sep="/"))
    kriglocations[kriglocations==minValue(kriglocations)] <- 0
    kriglocations[kriglocations==maxValue(kriglocations)] <- 1
